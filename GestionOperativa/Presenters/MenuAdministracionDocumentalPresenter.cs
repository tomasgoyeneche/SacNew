...
namespace GestionOperativa.Presenters
{
    public class MenuAdministracionDocumentalPresenter : BasePresenter<IMenuAdministracionDocumentalView>
    {
        ...
        public async Task VerificarLegajosChoferesAsync()
        {
            ...
            string rutaSeguroEmpresa = Path.Combine(rutaBase.Ruta, "EMPRESA", "ART"); // Ruta alternativa para seguros centralizados
            ...
            var subCarpetas = new Dictionary<string, string>
    {
        ...
        { "Licencia", "Licencia" }
            ...
            // 🔹 Delegamos la generación de archivos faltantes en un solo método
            Func<ChoferDto, List<string>> obtenerArchivosFaltantes = chofer =>
            {
                ...
                // 🔹 Verificar seguro en base a CoberturaCentralizada
                string rutaSeguro = chofer.CoberturaCentralizada
                    ? Path.Combine(rutaSeguroEmpresa, $"{chofer.Empresa_Cuit}.pdf") // Si es centralizado, usa CUIT
                    : Path.Combine(rutaRaiz, "Seguro", $"{chofer.Documento}.pdf"); // Si no, usa Documento
        ...
        public async Task VerificarLegajosEmpresasAsync()
        {
            ...
            // 🔹 Método para obtener archivos faltantes
        ...
        {
            var rutaBase = await _confRepositorio.ObtenerRutaPorIdAsync(6); // IDCONF 2 para Q:\Fotos\
            if (rutaBase == null || string.IsNullOrEmpty(rutaBase.Ruta))
            {
