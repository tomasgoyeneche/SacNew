...
namespace Core.Repositories
{
    public class PeriodoRepositorio : BaseRepositorio, IPeriodoRepositorio
    {
        ...
        public async Task<List<Periodo>> ObtenerPeriodosActivosAsync()
        {
            var query = "SELECT * FROM Periodo WHERE Activo = 1";

            return await ConectarAsync(async connection =>
            {
                var result = await connection.QueryAsync<Periodo>(query);
        ...
        public async Task<int?> ObtenerIdPeriodoPorMesAnioQuincenaAsync(int mes, int anio, int quincena)
        {
            var query = @"
            SELECT IdPeriodo
            FROM Periodo
            WHERE Mes = @Mes AND Anio = @Anio AND Quincena = @Quincena";

            return await ConectarAsync(async conn =>
            {
                return await conn.QueryFirstOrDefaultAsync<int?>(query, new { Mes = mes, Anio = anio, Quincena = quincena });
            });
        }
